...
methods
    ...
    %already known due to the subtreeSizes array.
        ...
        if(m1~=m2)
            error('Inconsistent numers of rectangles given.');
        end
        ...
        if(exist('kdTreeCPPInt','file'))
            ...
            numInRange=kdTreeCPPInt('rangeCount',theTree.CPPData,rectMin,rectMax,m1);
        else
    ...
    %              (according to the squared l2 norm) of the given point
...
function val=rectsIntersect(rectMin1,rectMax1,rectMin2,rectMax2)
...
function val=inHyperrect(P,rectMin,rectMax)
%%INHYPERRECT Determine whether a point is within a given hyperectangular
%             region.
%
%INPUTS:    P   A kX1 vector.
%       rectMin A kX1 vector of the lower bounds of each of the
%                 dimensions of the k-dimensional hyperrectangle.
...
%December 2013 David F. Crouse, Naval Research Laboratory, Washington D.C.
    ...
    if(sum(P<rectMin)>0||sum(P>rectMax)>0)
        val=false;
    else
        val=true;
    end
