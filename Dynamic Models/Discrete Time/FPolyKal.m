...
%raised to a matrix in Chapter 4 of
%P. Zarchan and H. Musoff, Fundamentals of Kalman Filtering: A Practical
%Approach, 3rd ed. Reston, VA: American Institute of Aeronautics and 
%Astronautics, Inc., 2009.
%By examining how the state transition matrices evolve as the order goes
%up, (each unit increase in order means one more term in the Taylor series
%expansion remains) one can obtain a general rule for an arbitrary
%dimensionality. For numDim=1, the first row is T raised to powers from 0
%to order and those terms are divided by the factorial of their exponent.
%For example, T^n/factorial(n) for the nth column in the first row. The
%other rows are just the same as the first, but right-shifted (and zero
%padded).
%
