function Q=QPolyKalDirectAlt(T,x,order,sigmaV2)
...
%               distance^2/time^(2*order). sqrt(sigmaV2) is
%               proportional to the standard deviation of the noise
%               affecting the highest-order moment over a sampling period
%               of length T. If a scalar is passed, it is assumed to be the
%               same for all dimensions. Otherwise, a numDimX1 or 1XnumDim
%               vector should be passed specifying the value for each
%               dimension.
%
%OUTPUTS: Q     The process noise covariance matrix under the direct discrete
%               linear dynamic model of the given order with motion in
...
%present the discrete Wiener process acceleration for order 2. The logic
%behind model is also explained in Chapter 6.3.3 of
