...
%               means just position.
%
%Chapters 1.5.5 and 1.5.6 of
%Y. Bar-Shalom, P. K. Willett, and X. Tian, Tracking and Data Fusion.
%Storrs, CT: YBS Publishing, 2011.
%present the discrete white noise acceleration and jerk models, for orders
%one and two. The logic behind the order 1 model, which extends to any
%order, is explained in Chapter 6.3.2 of
...
%The generalization has the matrix for 1D motion being sigmaV2*G*G' where
%the ith element in the column vector G is T^(order-i+2)/(order-i+2)! where
%i counts from 1 to order+1.
%
