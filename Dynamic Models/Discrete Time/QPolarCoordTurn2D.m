...
%on the unnumbered equation on the second page of 
...
%which is page 436 of the proceedings. The model essentially puts noise on
%the turn rate component and allows it to integrate into the heading
%component; it also puts a noise term on the speed. When a linear
%acceleration term is present, then one could extend the concept to putting
%a noise term on the linear acceleration and letting it integrate into the
%speed. In the instance where a transverse acceleration is given, one can
%approximate the conversion based on the fact that omega*v=at and scaling
%the covariance terms appropriately from the model used in Blackman.
%
%Blackman's paper has a mysterious c_Q term, which he just set to 1. Upon
...
%noise for the model as per Section 8.2.2 of the book
...
%when considering that the Integrated Ornstein Uhlenbeck model has one
%level of integration less than the Singer model. Similar things can be
%done when the turn is represented in terms of a transverse acceleration
%and not a turn rate.
%
%Note that Blackman's discrete-time model does not integrate the errors
