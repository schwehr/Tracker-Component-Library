function [xVals,exitCodes]=homotopySolver(x0,f,FJacob,c,systemType,startForward,maxSol,initStepSize,order,solutionChoice,RelTol,AbsTol,maxSteps,lambdaTol,numStepsBeforeRestart)
%%HOMOTOPYSOLVER Use a probability-1 homotopy method to either find a fixed
%                point (f(x)=x)of a real, scalar or vector function or to
%                find when the function equals a certain value. Such
...
%                significantly larger than Newton's method. The success is
%                "probability 1", because given a completely random
%                initializer (over the entire real space), in the absence
%                of finite precision errors, the algorithm will converge to
%                the correct solution (if it exists) with probability 1. In
%                reality, precision errors can cause the algorithm to fail,
%                though more liekly the algorithm will fail, because a
...
function [xVal,exitCode,s,deltaS,u,dudsPrev]=takeOneStep(a,s,deltaS,u,dudsPrev,f,FJacob,c,systemType,order,solutionChoice,RelTol,AbsTol,maxSteps,lambdaTol,numStepsBeforeRestart)
    ...
    %try-catch frameowrk will catch all actual errors in the code that
