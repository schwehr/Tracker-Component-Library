...
%
%The algorithm is mostly taken from Chapter 3.3 of
%D. Bertsimas and J. N. Tsitsiklis, Introduction to Linear Optimization.
%Belmont, MA: Athena Scientific, 1997.
%and has not been optimized for sparse matrices. Thus, on large, sparse
%problems, the algorithm can be slow. Bland's rule from Chapter 3.4 is used
%to prevent cycling. The introduction of comparisons to epsilon rather than
%zero is not in the book, but was found to be necessary for even small
%problems. Preprocessing to handle inequality constrains by adding slack
%variables and turning them into equality constraints is not described in
%the text. The method of Chapter 3.5 for finding an intial feasible
%solution is used.  Chapter 3.3 describes periodically recomputing the
%inverse basis matrix from the actual bases to avoid accumulating roundoff
%errors. This is done every 30th iteration. Finite precision problems can
%potentially lead to redundant basis vectors being added. If this occurs,
%then the algorithm reports as failing.
%
...
function [x,basisIdx,exitFlag,foundSol]=solveSimplexGivenBasis(A,b,c,x,basisIdx,epsilon,maxIter)
    ...
    for curIter=1:maxIter
        %Step 2 in the book.
        %The components of the cost function associated with the chosen bases.
        cB=c(basisIdx); 
        p=cB'*BInv;
        ...
        negRedCostIdx=[];
        for curRedCost=1:n
        %We are using Bland's rule to avoid cycling, as mentioned at the end of
        %Chapter 3.4. That means, choose the first index that satisfies the
        %criterion.
        ...
        %If none of the reduced costs are negative, then the optimal solution
        %in basis form has been found.
        ...
        for i=1:m
            if(u(i)>=epsilon)
                ...
                %Since this is less than and not less than or equal to, this
                %satisfies the criterion for Bland's rule to avoid cycling.
                if(ratio<theta)
                    theta=ratio;
                    minIdx=i;
                end
