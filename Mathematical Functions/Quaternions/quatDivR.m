function Q=quatDivR(quat2,quat1,handed)
%%QUATDIVR Right-divide the quaternion quat2 by the quaternion quat1.
%          Quaternion algebra is not commutative, so a difference exists
%          between left and right division. Quaternions are an extension
%          of complex numbers. The quaternions can also be either left or
%          right-handed.
%
...
%       quat1 A 4XN set of N quaternions by which the corresponding
%             quaternion in quat2 is to be right-divided.
...
%           Q(:,i)=quat2(:,i)/quat1(:,i), where the division is right
%           quaternion division.
%
...
Q=quatMult(quat2,quatInv(quat1),handed);
end
