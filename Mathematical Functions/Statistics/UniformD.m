...
function x=rand(bounds)
%%RAND   Generate a multivariate uniform random variable with a given
%        set of bounds
%
%INPUTS:bounds  A 2XnumDim matrix of the minimum and maximum bounds of each
%               of the numDim dimensions of the uniform distribution.
...
%               the set of maximum bounds.
%
%OUTPUT: x   A sample of an numDimX1 random variable with the given bounds.
%
%March 2015 David F. Crouse, Naval Research Laboratory, Washington D.C.

    xDim=size(bounds,2);

    x=(bounds(2,:)-bounds(1,:))'.*rand(xDim,1)+bounds(1,:)';
end
