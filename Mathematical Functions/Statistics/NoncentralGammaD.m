classdef NoncentralGammaD
%Functions to handle the noncentral gamma distribution.
%Fully implemented methods are: PDF, CDF
%Partially implemented methods are: mean, var, invCDF, rand
%
%Computations in this class are based off of the following papers:
%
%I.R.C. de Oliveria and D.F. Ferreira, "Computing the noncentral gamma
%distribution, its inverse and the noncentrality parameter," Computational
%Statistics, vol. 28, no. 4, pp.1663-1680, 01 Aug 2013.
%
%L. Knüsel adn B. Bablok, "Computation of the noncentral gamma
...
%       parameters.
%
%INPUTS:    k       The shape parameter of the distribution.
%           theta   The scale parameter of the distribution.
%           lambda  The noncentrality parameter of the distribution
%
...
function val=PDF(x,k,theta,lambda,errtol)
%%PDF      Evaluate the noncentral gamma probability distribution function at a
%          desired point.
%
%INPUTS:    x       The point(s) at which the noncentral gamma PDF 
%                   is to be evaluated. Note that x>=0.
%           k       The shape parameter of the distribution.
%           theta   The scale parameter of the distribution.
%           lambda  The noncentrality parameter of the distribution
%           errtol Error tolerance for iteration. Default to eps.
%
%OUTPUTS:   val The value(s) of the PDF of the noncentral gamma
%               distribution with parameters k, theta, and lambda evaluated
%               at x.
%
%This is an implementation of the distribution computation from Oliveira
%and Ferreira. Knüsel and Bablok present approximations for the noncentral
%gamma distribution, and Oliveira and Ferreira present algorithms to
%compute these approximations. This function implements the algorithm for
...
function prob=CDF(x,k,theta,lambda,errtol)
...
%incomplete gamma function as described at
...
%            parameters.
%
%INPUTS:    N      If N is a scalar, then randGamma returns an NXN matrix
%                  of random variables. If N=[M,N1] is a two-element row 
%                  vector, then randGamma returns an MXN1 matrix of random
%                  variables.
%           k      The shape parameter of the distribution. This must be
%                  >0.
%           theta  The scale parameter of the distribution, which is >0.
%           lambda  The noncentrality parameter of the distribution
%
