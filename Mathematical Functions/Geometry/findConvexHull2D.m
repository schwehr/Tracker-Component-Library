...
%%FINDCONVEXHULL2D Given a set of points, find the convex polygon that just
%                  surrounds points in 2D. A form of Graham's algorithm,
%                  which has O(nlog(n)) complexity, is used. The vertices
%                  of the polygon (which coincide with points) are returned
%                  in counterlockwise order.
%
...
%We have to make sure that the problem was not degenerate after points were
%removed. That is, that we still have enough points to enter the loop.
    numPoints=size(points,2);
    if(numPoints<=2)
        vertices=points;
        return
