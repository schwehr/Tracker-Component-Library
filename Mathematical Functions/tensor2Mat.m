...
%           [1:(n-1),(n+1):N]. and the number of rows of the matrix Y
...
if(nargin<3)
    %This is the same as calling Y=tensor2Mat(X,R,C); with
    % N=length(I);%The total number of modes in the tensor.
    %R=n;
    %C=[(n+1):1:N,1:1:(n-1)];
