...
%               and its derivative are given. The values are assumed to be
%               given in increasing order.
%           y   A numMatchXNp matrix of values of the function y(x) and
%               its derivatives evaluated at values of the points in x.
%               y(row,col) corresponds to the row-1 derivative of y
%               evaluated at x(col).
%
%OUTPUTS: a       Coefficients of the interpolating polynomial of y in
%                 Newton form as a ROW vector (1XnumCoeff). The
%                 interpolating polynomial can be evaluated at a desired
%                 point using the function polyValNewton with a and the c
%                 point vector. By stacking multiple row vectors, one can
%                 use the function polyValNewton to evaluate multiple
%                 polynomials at the same time (i.e for vector valued
%                 functions).
%         c       Control points for the interpolating polynomial of y in
%                 Newton form as a 1XnumCoeff row vector.
%         aPower  Coefficients of the interpolating polynomial in the
%                 standard power series form scuh that interpolating at the
%                 point x is y(x)=aPower(end)+aPower(end-1)*x+aPower(end-1)*x^2+...
%                 as used by Matlab's polyder and polyval functions.
%
