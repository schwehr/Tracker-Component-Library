...
%(UNCLASSIFIED) DISTRIBUTION STATEMENT A. Approved for public release.
    ...
    y=linsolve(C,x,opts);
    %y=C\x;
    %The product y'*y, if x is a matrix, is also a matrix. Since loops are
    %slow in Matlab, presumably using diag(y'*y), which throws out the
    %unneeded off-diagonal elements is faster than using loops to find the
    %scalar y(:,i)'*y(:,i) terms directly and not actually computing the
