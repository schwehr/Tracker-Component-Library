...
Ais6::Ais6(const char *nmea_payload, const size_t pad)
  ...
  const ptrdiff_t payload_len = num_bits - 46;  // in bits w/o DAC/FI Crouse: Changed type from int to ptrdiff_t
...
Ais6_1_14::Ais6_1_14(const char *nmea_payload, const size_t pad)
  ...
  for (size_t window_num = 0; window_num < 3; window_num++) {
    ...
    w.y = (float)(sbits(bs, start, 27) / 600000.0);//Crouse: Added typecasting
    w.x = (float)(sbits(bs, start + 27, 28) / 600000.0);//Crouse: Added typecasting
    ...
    w.cur_speed  = (float)(ubits(bs, start + 86, 7) / 10.0);//Crouse: Added typecasting
...
Ais6_1_18::Ais6_1_18(const char *nmea_payload, const size_t pad)
  ...
  x = (float)(sbits(bs, 268, 25) / 60000.0);//Crouse: Added typecasting
  y = (float)(sbits(bs, 293, 24) / 60000.0);//Crouse: Added typecasting
...
Ais6_1_20::Ais6_1_20(const char *nmea_payload, const size_t pad)
  ...
  depth = (float)ubits(bs, 107, 8);//Crouse: Added typecasting
  ...
  x = (float)sbits(bs, 311, 25);//Crouse: Added typecasting
  y = (float)sbits(bs, 336, 24);//Crouse: Added typecasting
...
Ais6_1_32::Ais6_1_32(const char *nmea_payload, const size_t pad)
  ...
  for (size_t window_num = 0; window_num < 3; window_num++) {
    ...
    w.x = (float)(sbits(bs, start, 25) / 60000.0);//Crouse: Added typecasting
    w.y = (float)(sbits(bs, start + 25, 24) / 60000.0);//Crouse: Added typecasting
    ...
    w.cur_speed = (float)(ubits(bs, start + 80, 8) / 10.0);//Crouse: Added typecasting
    windows.push_back(w);
  }
