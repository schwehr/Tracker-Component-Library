...
Ais8_1_26_Curr3D::Ais8_1_26_Curr3D(const bitset<AIS8_MAX_BITS> &bs,
  ...
  for (size_t idx = 0; idx < 2; idx++) {
    ...
    currents[idx].north = (float)(ubits(bs, start, 8) / 10.0);//Crouse:Added typecasting
    currents[idx].east = (float)(ubits(bs, start + 8, 8) / 10.0);//Crouse:Added typecasting
    currents[idx].up = (float)(ubits(bs, start + 16, 8) / 10.0);//Crouse:Added typecasting
