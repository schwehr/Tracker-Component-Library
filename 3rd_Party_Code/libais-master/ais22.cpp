...
Ais22::Ais22(const char *nmea_payload, const size_t pad)
  ...
  if (!addressed) {
    ...
    pos_valid = true;
    dest_valid = false;
    x1 = (float)(sbits(bs, 69, 28) / 600000.0);//Crouse:Added typecasting
    y1 = (float)(sbits(bs, 87, 27) / 600000.0);//Crouse:Added typecasting
    x2 = (float)(sbits(bs, 104, 28) / 600000.0);//Crouse:Added typecasting
    y2 = (float)(sbits(bs, 122, 27) / 600000.0);//Crouse:Added typecasting
  } else {
    pos_valid = false;
    dest_valid = true;
