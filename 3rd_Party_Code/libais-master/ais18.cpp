...
Ais18::Ais18(const char *nmea_payload, const size_t pad)
  ...
  spare = ubits(bs, 38, 8);
  sog = (float)(ubits(bs, 46, 10) / 10.0);//Crouse:Added typecasting

  position_accuracy = bs[56];
  x = (float)(sbits(bs, 57, 28) / 600000.0);//Crouse:Added typecasting
  y = (float)(sbits(bs, 85, 27) / 600000.0);//Crouse:Added typecasting

  cog = (float)(ubits(bs, 112, 12) / 10.0);//Crouse:Added typecasting
