...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]) {
    ...
    double *xVec, TT1, TT2;
    double xp,yp;//The polar motion coordinates
    ...
    for(curVec=0;curVec<numVec;curVec++) {
        ...
        iauRxp(TIRS2ITRS, xVec+numRow*curVec, retData+numRow*curVec);
        
        //Multiply the velocity vector with the rotation matrix.
        if(numRow>3) {
            double *velTIRS=xVec+numRow*curVec+3;//Velocity in TIRS
            double *retDataVel=retData+numRow*curVec+3;//Velocity in TIRS
            
            //Convert velocity from TIRS to ITRS.
            iauRxp(TIRS2ITRS, velTIRS, retDataVel);
        }
    }
    }
