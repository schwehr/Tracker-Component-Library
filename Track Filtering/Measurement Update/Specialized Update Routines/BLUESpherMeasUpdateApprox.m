function [xStateUpdate,PUpdate,innov,S]=BLUESpherMeasUpdateApprox(xStatePred,PPred,z,R)
...
%
%The algorithm is the set of equations taken from Table I of
...
K=mu1*[lambda1*PBar(:,1),lambda1*PBar(:,3),PBar(:,5)]/S;

zCart=spher2Cart(z);
innov=zCart-mu1*[lambda1*xBar;lambda1*yBar;zBar];
xStateUpdate=xStateBar+K*innov;
PUpdate=PBar-K*S*K';
