function [xStateUpdate,PUpdate,innov,S]=BLUEPolarMeasUpdateApprox(xStatePred,PPred,z,R)
%%BLUEPOLARMEASUPDATEAPPROX Perform the measurmeent update step in the
...
%                 z A 2X1 monostatic polar measurement (no refraction)
%                   with components ordered [range;azimuth] with
%                   azimuth  in radians measured in radians\
...
%           innov, S    The 2X1 innovation and the 2X2 innovation
...
%
%The algorithm is taken from Section IV of
%Z. Zhao, X. R. Li, and V. P. Jilkov, "Best linear unbiased filtering with
%nonlinear measurements for target tracking," IEEE Transactions on
%Aerospace and Electronic Systems, vol. 40, no. 4, pp. 1324-1336,
%Oct. 2004.
%One equation that is not directly written in the above journal article is
%taken from the conference version
%Z. Zhao, X. R. Li, and V. P. Jilkov, "Optimal linear unbiased filtering
%with polar measurements for target tracking," in Proceedings of the 5th
%International Conference on Information Fusion, Annapolis, MD, 8?11 Jul.
%2002, pp. 1527-1534.
%
...
%from Section VI is taken from the form written out explicitely in Equation
...
S(1,1)=lambda2*covXTilde+lambda3*covYTilde+(1/2)*sigmaR2+lambda3*yBar^2+...
    (lambda2-lambda1^2)*xBar^2+(1/2)*sigmaR2*exp(-2*sigmaTheta2)*(-ExyRat1);
S(2,2)=lambda2*covYTilde+lambda3*covXTilde+(1/2)*sigmaR2+lambda3*xBar^2+...
    (lambda2-lambda1^2)*yBar^2+(1/2)*sigmaR2*exp(-2*sigmaTheta2)*(ExyRat1);
S(1,2)=exp(-2*sigmaTheta2)*covXTileYTilde+(exp(-2*sigmaTheta2)-...
...
xStateUpdate=xStateBar+K*(zCart-zCartPred);
PUpdate=PBar-K*S*K';
