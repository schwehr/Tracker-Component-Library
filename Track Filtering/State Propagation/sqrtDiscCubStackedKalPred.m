function [xPred, SPred]=sqrtDiscCubStackedKalPred(xPrev,SPrev,f,SQ,xi,w,stateDiffTrans,stateAvgFun,stateTrans)
%%SQRTDISCCUBSTACKEDKALPRED  Perform the discrete-time prediction step that   
%                 comes with the square root implementation of the cubature
%                 Kalman filter with non-additive process noise.
%
...
%                   that takes the stacked state and the process noise as
%                   its parameter. It returns the propagated state given
%                   the realization of the process noise.
%           SQ      The xDimX xDim lower-triangular square root of the 
%                   process noise covariance matrix.
%           xi      An (xDim+noiseDim) X numCubPoints matrix of cubature
%                   points for the joint state and process noise.
...
%                   covariance estimate.
%
%The mathematics behind the function sqrtDiscCubStackedKalPred are 
...
%(UNCLASSIFIED) DISTRIBUTION STATEMENT A. Approved for public release.
    ...
    for curP=1:numCubPoints
        xPropPoints(:,curP)=f(xStackedPoints(:,curP));
    end
