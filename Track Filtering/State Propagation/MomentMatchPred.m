...
%                able to handle sets of differences between state
%                estimates. For an xDim state, this must handle an xDimXN 
%                matrix of N differences. This only needs to be supplied if
%                the difference has to be restricted to a certain range.
%                For example, if elements of the state are angles, then
%                the difference between angles in two states should be
%                limited to +/-pi using the wrapRange function for that
%                element.
%
...
%F. Crouse. 
%The optional parameter stateDiffFunc is not described in the above
