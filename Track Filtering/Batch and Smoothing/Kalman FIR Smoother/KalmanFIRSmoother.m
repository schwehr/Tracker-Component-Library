...
%z(k)=H(:,:,k)*x(:,k)+noise
%where x is the target statem u is a control input, and z is the
%measurement.
%
%The algorithm is that given in
%D. F. Crouse, P. Willett, and Y. Bar-Shalom, "A low-complexity sliding-
%window Kalman FIR smoother for discrete-time models," IEEE Signal
%Processing Letters, vol. 17, no. 2, pp. 177-180, Feb. 2009.
%Note that if H, F, R, Q and kD are fixed, the matrices produced by the
%function KalmanFIRSmootherCoeffs, which is called by this function, can be
%calculated once and used repeatedly for different sets of measurements z
%and control inputs u. The matrix G in the paper is omitted, since any
