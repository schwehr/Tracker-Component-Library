function sourceLoc=TDOAOnlyStaticLocEst(refRxLocs,nonRefRxLocs,timeDelays,c)
...
%             problem to be observable.
%
%INPUTS: refRxLocs If there is only one reference receiver (the receiver
...
%               of the target.
%       timeDelays If only one reference receiver is present, this can be a
%               numRxX1 vector of the time delays between the reach of the
%               receivers (in nonRefRxLocs) and the reference receiver. In
%               the general case, this is a numRefsX1 cell array where the
%               ith cell holds the numRxiX1 vector of delays for the
...
%M. D. Gillette and H. F. Silverman, "A linear closed-form algorithm for
%source localization from time-differences of arrival," IEEE Signal
