...
%               fifthOrderCubPointsAlt is used to generate cubature points
%               if range rate is desired, and arbOrderCubPoints of order 5
%               is used if range rate is not desired.
...
%                measurement in [r;theta] polar coordiantes or as
...
%by David F. Crouse.
%However, a few changes had to be made to handle the circular nature of the
%measurements. The weighed average of the angular component was done using
%the meanAng function, and the differences in angle for computing the
%covariance matrix were also wrapped to +/-pi to handle the circular nature
...
%(UNCLASSIFIED) DISTRIBUTION STATEMENT A. Approved for public release.
    ...
    if(nargin<5)
        if(numDim==2)
            ...
            [xi,w]=fifthOrderCubPointsAlt(numDim);
        end
    end
